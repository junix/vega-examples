...
本仓库已经有 `src/35-pack-dendrogram` 讲**径向**树（`orient: "radial"` + 手工极坐标换算）。
...
# 浏览器打开 http://localhost:8000/src/44-tree-layout/
```
...
这是本 demo 和 `src/35-pack-dendrogram` 的对照点：
...
（本仓库校验器判失败）。本例照 `src/10-signals-bind/` 的做法改成派生 signal：
