...
# 浏览器打开 http://localhost:8000/src/43-edge-bundling/
```
...
const dir = './src/43-edge-bundling/';
...
- 本仓库相关 demo：`src/35-pack-dendrogram/`（同一份 `flare.json`，讲 `pack` 与 `treelinks`+`linkpath` 版的径向树）、
  `src/14-force-directed-graph/`（同一类网络数据的力导向画法）、
  `src/10-signals-bind/`（data 驱动 domain 的空态兜底写法）
