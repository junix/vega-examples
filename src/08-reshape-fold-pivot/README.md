...
| `marks[0].signals` | **局部 height 信号** | `{"name": "height", "update": "foldHeight"}`。`range: "height"` 的含义是「取当前作用域里名为 height 的信号」，group mark **不会**自动把它重绑为自己的高度 —— 少了这一条，y 轴会按顶层的 630 铺开，折线直接冲出分区（详见 src/05 的「作用域陷阱」一节） |
