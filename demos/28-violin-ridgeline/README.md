...
1. 用 `kde` 变换做**分组核密度估计**，并搞清它的五个关键参数：
   `groupby` / `bandwidth` / `counts` / `resolve` / `steps`（`minsteps`+`maxsteps`）。
...
| `data: ridge-density` | 12 条密度 + 反查均温 | `kde(resolve: "shared", steps: 200)` → 全体 `allMax` → `w` → `lookup` 把 `meanT` 贴回每一行（`area` 的 `fill` 只能读自己 datum 上的字段） |
| `data: temp-overall` | 全年均温 + 行数 | 不带 `groupby` 的 `aggregate` 就是全体统计；`count` 用来把「8,759 小时」写活（CSV 首行是表头、首条数据从 01:00 开始，所以不是 8760） |
| `scales: xcat / yMass / species` | 左面板 | `range` 一律写**显式区间** `[0, {"signal": "violinW"}]`，绕开 `range: "width"` 的作用域陷阱 |
| `scales: xTemp` | 右面板横轴 | domain 取 `ridge-density.value`（kde 的取样点），`nice: true` 后是 2–26 °C |
...
所以后面的 mark 编码里可以直接 `datum.w`。三种口径的实测宽度（默认带宽下，单位 px）：

| `widthMode` | `counts` | 分母 | Adelie(151) / Chinstrap(68) / Gentoo(123) |
| --- | --- | --- | --- |
| `count` | `true` | 全体最大 | **74 / 46 / 53** —— 面积∝样本量，谁的样本多谁胖 |
| `density` | `false` | 全体最大 | 55 / **74** / 49 —— 每组积分都是 1，谁的分布集中谁瘦高 |
| `equal` | `false` | 组内最大 | 74 / 74 / 74 —— 只比形状 |
...
### 关键概念四：`resolve` 决定曲线的两端收在哪

| `resolve` | 取样域 | 效果 | 取样点数 |
| --- | --- | --- | --- |
| `independent`（默认） | 每组自己的 `[min, max]` | 小提琴两端正好收在该组极值处，形状最紧凑 | 自适应，`minsteps`~`maxsteps` 之间 |
| `shared` | 全体数据的 `[min, max]` | 各组共用一个栅格，尾部拖出长长的近零段；但可以逐列比较、相减 | 固定 `steps`（此时 `minsteps = maxsteps`） |

山脊线用 `shared`：12 行必须共用一个横向栅格，视觉上才是「同一把尺子上的 12 条曲线」。
小提琴留成信号，可以现场对比两种效果（切到 `shared` 会看到 Chinstrap 的小提琴长出
一条细长的尾巴 —— 因为 Gentoo 的 6300 g 把取样域拉到了那里）。
...
**轴网格**靠 group 的局部信号。详见 `demos/05-stacked-grouped-bar/README.md` 的同名小节。
...
1. **拖 `bwMass`**：默认 0 = 自动，各组算出来是 180–205 g，三条曲线都是单峰。
   手动压到 **100 g**，Gentoo 立刻分出两个包（≈4750 g 与 ≈5500 g，正是雌雄两个众数）；
   继续压到 **20 g**，三个组分别抖出 16–20 个假峰；反过来拉到 **400 g**，
...
5. **把 `ridge-density` 的 `kde` 参数 `resolve` 改成 `"independent"`**：
   每个月只在自己的温度范围内取样，12 行的曲线不再左右对齐，
   横向比较立刻变难 —— 这就是山脊线必须用 `shared` 的原因。
...
第三方 `joypy` 能画山脊线，但它是薄封装：换配色、改重叠、加参考线都得回到
